...
const DEFAULT_API = "https://api-gateway.greenisland-18bb041c.southeastasia.azurecontainerapps.io";
...
function App() {
  ...
  async function api(path, options = {}) {
    // All API calls go through the gateway which handles CORS and routing
    return requestJson(DEFAULT_API, path, options);
  }
  ...
  const refreshProducts = useCallback(async (showSpinner = false) => {
    ...
    try {
      const productsResult = await api("/inventory");
